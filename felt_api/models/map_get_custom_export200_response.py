...
class MapGetCustomExport200Response(BaseModel):
    """
    ...
    filters: List[Any]
    status: StrictStr
    __properties: ClassVar[List[str]] = ["download_url", "export_id", "filters", "status"]

    @field_validator('status')
    def status_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['completed', 'failed', 'in_progress']):
            raise ValueError("must be one of enum values ('completed', 'failed', 'in_progress')")
        return value
