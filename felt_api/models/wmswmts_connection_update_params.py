...
class WMSWMTSConnectionUpdateParams(BaseModel):
    """
    Web Map Service / Web Map Tile Service
    """ # noqa: E501
    type: StrictStr
    url: Optional[StrictStr] = Field(default=None, description="WMS/WMTS URL")
    __properties: ClassVar[List[str]] = ["type", "url"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['wms_wmts']):
            raise ValueError("must be one of enum values ('wms_wmts')")
        return value
